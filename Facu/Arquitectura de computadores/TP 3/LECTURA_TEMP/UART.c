...
void USART_send( unsigned char data){
	while(!(UCSR0A & (1<<UDRE0))); //espera mientras el registro esté lleno
	UDR0 = data; //se asigna el valor a transmitir al registro de datos
}
unsigned char USART_receive(void){
	while(!(UCSR0A & (1<<RXC0))); //espera mientras el flag RX sea cero
	return UDR0; //si el flag es uno devuelve el contenido del registro de datos
}
void USART_putstring(char* StringPtr){
	while(*StringPtr != 0x00){ //Se chequea si no hay más caracteres para enviar
		USART_send(*StringPtr); //usando esta simple función se envía de a un caracter
